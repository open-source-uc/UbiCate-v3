...
export async function POST(request: NextRequest) {
  try {
    ...
    // Update the ubicacion_geografica estado to 1 (Rechazado)
    const updateSql = `
      UPDATE ubicacion_geografica 
      SET fk_id_estado_ubicacion_geografica = 1 
    ...
    logger.info(`[API] Ruta rechazada ID: ${id_ruta} por usuario: ${nombreUsuario}`);
    return NextResponse.json({ 
      success: true, 
      message: "Ruta rechazada exitosamente" 
    });

  } catch (error) {
    logger.error("Error rejecting route:", error);
    return NextResponse.json(
